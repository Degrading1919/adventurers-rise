"""Run server Gathering modules with trusted Inventory, Equipment, and Skills boundaries."""
...
def main():
    ...
    modules = {
        "GatheringNodeIds": root / "src/Shared/GatheringNodeIds.luau",
...
local fakeGame = {
    GetService = function(_, name)
        ...
        return { Shared = { GatheringNodeIds = "GatheringNodeIds", EquipmentSlots = "EquipmentSlots", ItemDefinitions = "ItemDefinitions", SkillIds = "SkillIds" } }
    end,
}
local loaded = {}
...
''')
    wrappers.append((root / "tests/Gathering.spec.luau").read_text(encoding="utf-8"))
    wrappers.append("end\nrunTests(moduleRequire)\n")

    with tempfile.TemporaryDirectory(prefix=".gathering-", dir=root / "tests") as directory:
        ...
        for name, path in modules.items():
            ...
            source = source.replace('require(Shared.GatheringNodeIds)', 'require("./GatheringNodeIds")')
