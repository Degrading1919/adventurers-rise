"""Run server OfflineTraining modules against Skills and Plot/Training boundaries."""
...
def main():
    ...
    shared_modules = ["SkillIds", "PlotSockets", "EquipmentSlots", "ItemDefinitions"]
    modules = {
...
''')
    wrappers.append((root / "tests/OfflineTraining.spec.luau").read_text(encoding="utf-8"))
    wrappers.append("end\nrunTests(moduleRequire)\n")

    with tempfile.TemporaryDirectory(prefix=".offline-training-", dir=root / "tests") as directory:
