"""Run the StarterKit ModuleScript against the real Inventory/Equipment and a fake PlayerData."""
...
def main():
    ...
    shared_names = ["EquipmentSlots", "ProgressionFlagIds", "ItemDefinitions"]
    ...
    modules.update({
        ...
        "StarterKitService": root / "src/Server/StarterKit/Service.luau",
    })
...
''')
    wrappers.append((root / "tests/StarterKit.spec.luau").read_text(encoding="utf-8"))
    wrappers.append("end\nrunTests(moduleRequire)\n")

    with tempfile.TemporaryDirectory(prefix=".starterkit-", dir=root / "tests") as directory:
