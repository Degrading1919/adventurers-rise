"""Run the pure region/zone definitions + ZoneAt classifier behaviour checks.

RegionDefinitions is pure shared data with no Roblox instances, so it is fully analyzable and testable
headlessly. Whole-source luau-compile below covers syntax; the behaviour is exercised against the real
modules with the standard require shim.
"""
...
''')
    wrappers.append((root / "tests/RegionDefinitions.spec.luau").read_text(encoding="utf-8"))
    wrappers.append("end\nrunTests(moduleRequire)\n")

    with tempfile.TemporaryDirectory(prefix=".region-", dir=root / "tests") as directory:
