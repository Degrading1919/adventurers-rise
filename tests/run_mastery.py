"""Run server Mastery definitions/service modules against Skills and Economy boundaries."""
...
def main():
    ...
    shared_modules = ["MasteryIds", "SkillIds", "EquipmentSlots", "ItemDefinitions"]
    modules = {
        "MasteryIds": root / "src/Shared/MasteryIds.luau",
...
''')
    wrappers.append((root / "tests/Mastery.spec.luau").read_text(encoding="utf-8"))
    wrappers.append("end\nrunTests(moduleRequire)\n")

    with tempfile.TemporaryDirectory(prefix=".mastery-", dir=root / "tests") as directory:
