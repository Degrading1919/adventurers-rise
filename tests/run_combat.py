"""Run the server Combat ModuleScript against trusted Equipment, Skills, and target boundaries."""
...
local fakeGame = {
    GetService = function(_, name)
        ...
        return { Shared = { EquipmentSlots = "EquipmentSlots", ItemDefinitions = "ItemDefinitions", SkillIds = "SkillIds" } }
    end,
}
local loaded = {}
...
''')
    wrappers.append((root / "tests/Combat.spec.luau").read_text(encoding="utf-8"))
    wrappers.append("end\nrunTests(moduleRequire)\n")

    with tempfile.TemporaryDirectory(prefix=".combat-", dir=root / "tests") as directory:
