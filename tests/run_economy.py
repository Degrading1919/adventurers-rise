"""Run Economy ModuleScripts against a minimal trusted PlayerData boundary."""
...
local fakeGame = {
    GetService = function(_, name)
        ...
        return { Shared = { EquipmentSlots = "EquipmentSlots", ItemDefinitions = "ItemDefinitions" } }
    end,
}
local loaded = {}
...
''')
    wrappers.append((root / "tests/Economy.spec.luau").read_text(encoding="utf-8"))
    wrappers.append("end\nrunTests(moduleRequire)\n")

    with tempfile.TemporaryDirectory(prefix=".economy-", dir=root / "tests") as directory:
