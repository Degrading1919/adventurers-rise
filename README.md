...
- [`docs/PLAYER_DATA.md`](docs/PLAYER_DATA.md) and [`docs/SKILL_XP.md`](docs/SKILL_XP.md) — implemented persistence and server Skill/XP boundaries
...
Both entry points load the shared, immutable project name and report startup in Output. The server also starts versioned PlayerData loading, autosave, and session cleanup. Server-only Skill/XP modules derive levels from PlayerData XP once a future balance task supplies their explicit curve configuration; they do not add XP triggers or client access. See [`docs/PLAYER_DATA.md`](docs/PLAYER_DATA.md) and [`docs/SKILL_XP.md`](docs/SKILL_XP.md) for their boundaries and repository checks. No system loader, remotes, gameplay, or external dependencies are installed.
