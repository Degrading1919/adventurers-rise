...
## CURRENT PHASE: Phases 1–3 DONE + evaluator-cleared. Phase 4 — 4a (Vanguard) + 4b (discovery+map) + 4c (route-the-player + fixes) DONE + verified. Phase-4 evaluator done (verdict: route, don't add Mastery). Remaining Phase-4 recs: Vanguard payoff, then activate dead Mastery branches; then Phase 5.
...
- **4d — Vanguard visible payoff (evaluator #2, NEXT):** give each Ascension rank a real, surfaced reward (e.g. a permanent MaxHealth or damage-reduction perk per rank, folded into the existing modifier providers like armor was; surfaced in the read model + Ascend panel), so Ascending rewards rather than relabels. Data-driven per rank. Test + Studio-verify.
- **4e — Activate the dead Mastery branches (evaluator #3):** wire the 4 currently-inert branches (MeleeCleave, DefenseResilience, MiningProspecting, SmithingMerchantSmith) to real effects instead of adding a 4th tier — start with Melee Cleave (needs a multi-target hook) as the meatiest. Makes existing choices matter (criterion 4). Larger; may slip to a later phase.
